...
int i;         //loop index
 ...
 dart=drand48()*ttl-v[0];  //throw dart
  ...
  while((i<N)&&(dart>0))dart-=v[++i];  //figure out where the dart landed
...
int i,j;
  ...
  for(i=0;i<n;i++)if(mem[i]==z){//found it
...
int i;            //index
  ...
  if(ttl>0.01){
    ...
    i=0;dart-=ft[mem[0]];    //hit first?
    while((dart>0)&&(i<n)){  //iterated:
      i++;dart-=ft[mem[i]];  //hit next?
    }
...
int k;
  ...
  inp.getline(buf,59);
  n=atoi(buf);
  k=0;while(buf[k]!=' ')k++;while(buf[k]==' ')k++;max=atoi(buf+k);
  mem=new int[max];
  for(k=0;k<n;k++){
    inp.getline(buf,59);
    mem[k]=atoi(buf);
...
graph::graph(){//initialize an empty structure
  ...
  for(int i=0;i<MAXQ;i++){quality[i]=0;weights[i]=0;}  //zero out weights,quals
...
int i;  //loop index
  ...
  for(i=0;i<nqual;i++){delete [] quality[i];quality[i]-0;} //clear quals
  nqual=0;                                                 //record no quals
  for(i=0;i<nwgt;i++){delete [] weights[i];weights[i]-0;}  //clear weights
...
void graph::RecordQ(int num,int *Q){//assign Q to the quality in question
  ...
  if((num<0)||(num>=nqual))return;  //try to initialize a non-existant quality
...
void graph::RetrieveQ(int num,int *Q){//get the values in the quality -> Q
  ...
  if((num<0)||(num>=nqual))return;  //try to retrieve a non-existant quality
...
void graph::RecordW(int num,double *W){//assign W to the weights in question
  ...
  if((num<0)||(num>=nwgt))return;  //try to retrieve a non-existant weight
...
int i,j;
  ...
  if((M==0)||(M<n))create(n);  //make sure storage is available
  V=n;E=n*(n-1)/2;             //create values for V and E
...
int i,j;
  ...
  V=n+m;E=n*m;                    //create values for V and E

  for(i=0;i<n;i++)for(j=0;j<m;j++){//loop over relevant pairs
    nbr[i].add(n+j);
    nbr[n+j].add(i);
  }
...
int ls[2];
  ...
  if((M==0)||(M<n))create(n);  //make sure storage is available
  V=n;E=n;                     //create values for V and E
  for(i=0;i<n;i++){
    ls[0]=(i+1)%n;
    ls[1]=(i+n-1)%n;
    nbr[i].create(ls,2);
...
int i;
  ...
  V=2*n;E=3*n;  //addign the vertex and edge values
  for(i=0;i<n;i++){
...
int i,j;
  ...
  V=n;E=m*n;  //adding the vertex and edge values
  for(i=0;i<n;i++){
    for(j=1;j<=m;j++){
      nbr[i].add((i+j)%n);
      nbr[i].add((i-j+n)%n);
...
int i,j,k;  //loop index variables
  ...
  clearE();
  k=0;
  for(i=0;i<V-1;i++)for(j=i+1;j<V;j++){
    if(ed[k++]==1){
      nbr[i].add(j);
      nbr[j].add(i);
      E++;
...
int i;
  ...
  clearE();
  for(i=0;i<wl-1;i++)if(edgeP(wk[i],wk[i+1])==0)toggle(wk[i],wk[i+1]);
...
int c;     //command index
  ...
  for(c=0;c<L;c++){//loop over commands
    switch(cs[c][0]%5){
    case 0: //Hop
      hop(cs[c][1]%V,cs[c][2]%V,cs[c][2]/V);  //assume large integer rep
      break;
    case 1: //Add
      add(cs[c][1]%V,cs[c][2]%V);
      //cout << "ADD" << endl;
...
int dx[n];       //shuffled vertices
  ...
  V=n;E=1; //assign the vertex and edge values
  ...
  for(i=1;i<n;i++){ //add vertices one by one
    ...
    for(j=0;j<temp.size();j++) p.push_back(temp.memz(j)); //add new connections
...
if((M==0)||(M<n))create(n); //make sure storage is available
V=n; E=0; //start with n vertices and no edges

  do{
    for(i=0;i<n-1;i++) for(j=i+1;j<n;j++) {//cycle through all possible edges
...
int d1,d2;    //degrees of neighbors
  ...
  if((n1<0)||(n1>=d1))n1=(n1%d1+d1)%d1;  //force possible neighbor
  ...
  if((n2<0)||(n2>=d2))n2=(n2%d2+d2)%d2;  //force possible neighbor, again
...
int *temp;
  ...
  for(i=tt;i<tt+qq-1;i++)for(j=tt;j<tt+qq-1;j++)if(i!=j)nbr[i].add(j);
...
int n1,n2;  //decoded neighbors
  ...
  v1=a%V;if(nbr[v1].size()<=k)return; //check degree bound, first vertex
  v2=b%V;if(nbr[v2].size()<=k)return; //check degree bound, second vertex
...
int graph::edgeP(int a,int b){//
  ...
  if((a<0)||(b<0)||(a>=V)||(b>=V))return(0);  //non-vert are non-edge
...
int qq,vv; //scratch variables
  ...
  for(i=0;i<V;i++)ds[i]=-1;  //negative one is the surrogate for infinity
  ds[z]=0;
  cd=0;
  do{
    fl=0;
    for(i=0;i<V;i++)if(ds[i]==cd){//if we are at current distance
...
int b;       //origin loop index
  ...
  for(i=0;i<V;i++){//loop over the vertices
    if(q[i]==-1){delete [] q;return(-1);} //the graph is disconnected
...
int i,rv;
  ...
  for(i=0;(i<V)&&(rv==1);i++)if(q[i]==-1)rv=0;  //evidence of disconnection
  delete [] q;        //delete distance array
  return(rv);
...
int *ds;
  ...
  if(connectedP()==1){
    ...
    for(i=0;i<V;i++){   //loop over vertices
      ...
      for(j=0;j<V;j++)if(ds[j]>ecs[i])ecs[i]=ds[j]; //find eccentricity
    }
    delete [] ds;
...
int *nin;   //number of infected neighbors
  ...
  while(NI>0){//while there is still an epidemic
    ...
    for(i=0;i<V;i++)nin[i]=0; //zero the number of infected neighbors buffer
    for(i=0;i<V;i++)if(clr[i]==1){//found infected individual  
      for(j=0;j<nbr[i].size();j++)nin[nbr[i].memz(j)]++; //record exposure
    }
    //check for transmission
    for(i=0;i<V;i++)if((clr[i]==0)&&(nin[i]>0))clr[i]=3*infected(nin[i],alpha);
    if(NI>max)max=NI; //check for updated maximum
    ttl+=NI; //add the infected to the total
    NI=0;  //zero the number infected counter
    for(i=0;i<V;i++)switch(clr[i]){//status update
...
int scratch[V]; //new qualities
  ...
  for(i=0;i<V;i++){//loop over vertices
	...
	else {scratch[i]=1;nsp++;}         //preserve and record infection
      break;
    case 2: //removed
...
double dart;    //used to resolve multiple probabilities
  ...
  for(i=0;i<V;i++){//loop over vertices
	...
	else {scratch[i]=2;nsp++;}         //preserve and record infection
      break;
    case 3: //removed
...
double dart;    //used to resolve multiple probabilities
  ...
  for(i=0;i<V;i++){//loop over vertices
	...
	else {scratch[i]=3;nsp++;}         //preserve and record infection
      break;
    case 4: //removed
...
double dart;    //used to resolve multiple probabilities
  ...
  for(i=0;i<V;i++){//loop over vertices
    ...
    case 4: //Long term asymptomatic
        dart=drand48();  //throw the dart
        if(dart<gamma)scratch[i]=5;   //remove!
        else {scratch[i]=4;nsp++;}    //preserve and record infection
      break;
    case 5: //removed
        dart=drand48();  //throw the dart
...
int graph::attack(double *pr){//probabalistic attack method
  ...
  if((M==0)||(V==0))return(0); //treat empty graphs as totally vulnerable
...
int v;            //vertex buffer
  ...
  for(i=0;i<V;i++){//loop over vertices
    ...
    for(j=0;j<tg;j++)if(used[j]==0)aval[k++]=j; //compile available colors
    //cout << "K=" << k << endl;
...
int k;
  ...
  inp.getline(buf,999);
  M=atoi(buf);
  k=0;while(buf[k]!=' ')k++;while(buf[k]==' ')k++;
  V=atoi(buf+k);    
  while(buf[k]!=' ')k++;while(buf[k]==' ')k++;
  E=atoi(buf+k);
  nbr=new set[M];
