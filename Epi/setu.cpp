...
{ // sum indecies of ft in the set
    ...
    for (i = 0; i < n; i++)
        ttl += ft[mem[i]]; // sum the indexed members
...
{ // create with max maximum vertices
    ...
    if (M != 0)
        destroy(); // clear the graph if it is not empty
...
{ // index of quality or -1 if none left
    ...
    for (int i = 0; i < V; i++)
        quality[nqual][i] = v; // initialize to v
...
{ // as above, but initialize to Q -- size==V
    ...
    quality[nqual] = new int[V]; // create an integer register for each vertex
    for (int i = 0; i < V; i++)
        quality[nqual][i] = Q[i]; // initialize to Q
    nqual++;                      // record that quality was allocated
    return (nqual - 1);           // return index of quality
}
...
{ // get the values in the quality -> Q
    ...
    for (int i = 0; i < V; i++)
        Q[i] = quality[num][i]; // dump values into Q
}
...
{ // index of quality or -1 if none left
    ...
    for (int i = 0; i < V; i++)
        weights[nwgt][i] = v; // initialize to v
...
{ // as above but initialize to W -- size==V
    ...
    weights[nwgt] = new double[V]; // create an double register for each vertex
    for (int i = 0; i < V; i++)
        weights[nwgt][i] = W[i]; // initialize to W
    nwgt++;                      // record that quality was allocated
    return (nwgt - 1);           // return index of quality
}
...
{ // get the values in the quality -> W
    ...
    if ((num < 0) || (num >= nwgt))
        return; // try to retrieve a non-existant weight
    for (int i = 0; i < V; i++)
        W[i] = weights[num][i]; // transfer weights to W
}
...
{ // complete bipartite
    ...
    if ((M == 0) || (M < n + m))
        create(n + m); // make sure storage is available
    V = n + m;
...
{ // Petersen n,m
    ...
    if ((M == 0) || (M < 2 * n))
        create(2 * n); // make sure storage is available

    if (m > n)
        m %= n; // failsafe the arithmetic
...
{ // hop an edge
    ...
    // cout << v << " " << nb1 << " " << nb2 << endl;
    if (edgeP(v, nb2) == 1)
        return; // no hop possible - its a triangle
    if (v == nb2)
...
{ // simplexify at a
    ...
    if (qq <= 1)
        return; // nothing happens in this case

    int i, j; // loop index variables
    int *temp;

    if (M - V < qq)
        Enlarge(M + qq); // if more space is needed, make it

    tt = V;        // record lower bound on new vertices
    V += (qq - 1); // indicate that the new vertices exist
    temp = new int[qq];
    for (i = 0; i < qq; i++)
        temp[i] = nbr[a].memz(i); // save neighbots of i
...
{ // hop an edge
    ...
    // cout << v << " " << nb1 << " " << nb2 << endl;
    int num_edges = nbr[v].ElementCount(nb2);
    if (edgeP(v, nb2) == 1 && num_edges == maxWeights)
        return; // no hop possible - its a triangle
...
{ // decode and perform an edge swap
    ...
    if (V < 4)
        return; // we need to have four vertices
    ...
    v1 = a % V;
    if (nbr[v1].size() <= k)
        return; // check degree bound, first vertex
    v2 = b % V;
    if (nbr[v2].size() <= k)
        return; // check degree bound, second vertex
    if (nbr[v1].memb(v2))
        return;                    // added edge in quartet
...
{ // eccentricity of a vertex
    ...
    if (M == 0)
        return (0); // empty graphs yields no eccentricity
    if (V == 0)
        return (0); // like it says
...
{ // is the graph connected?
    ...
    if (M == 0)
        return (1); // empty graphs are vacuously connected
    if (V == 0)
        return (1); // like it says
    q = new int[V]; // create distance array
...
{ // diffusion character at v
    ...
    { // loop over time steps
        ...
        for (i = 0; i < V; i++)
            dc[i] = omega * sm[i]; // apply the decay term
...
{ // one time step
    ...
    for (i = 0; i < V; i++)
        RecordQ(q, i, scratch[i]); // update the quality
...
{ // probabalistic attack method
    ...
    { // knock out vertices
        ...
        if (i >= V)
            break;    // D'oh  Ate the whole graph
...
{ // read the graph
    ...
    if (M != 0)
        destroy(); // clear the graph if it is not already clear
    inp.getline(buf, 999);
    M = atoi(buf);
    k = 0;
    while (buf[k] != ' ')
